...
export const formToJson = target => {
    const form = target;
    const serialized = $(form).serializeArray();

    const data = serialized.reduce((acc, { name, value }) => {
        const inputElement = $(form).find(`[name="${name}"]`);
        if (inputElement.attr('type') === 'date' && value) {
            acc[name] = new Date(value).toISOString();
        } else {
            acc[name] = value.trim(); // Trim last space in values
        }
        return acc;
    }, {});

    // Recorre todos los inputs para garantizar que estén en el objeto
    $(form)
        ...
        .each(function () {
            ...
            if (!name) return; // Saltar elementos sin atributo 'name'

            if (!(name in data)) {
                if (inputElement.is(':checkbox')) {
                    data[name] = inputElement.prop('checked');
